...
library(caret)
library(lubridate)
energydemand <- read_excel("datasets/charging_profile_regular.xlsx")
energydemand$day <- lubridate::as_date(ymd_hms(energydemand$date_time))

lubridate::as
demand<- energydemand %>% group_by()
...
grouped_month <- energydemand %>%
  group_by(month) %>% 
  summarise(avg_pubpower = mean(power_demand_public), sd_pubpower = sd(power_demand_public))
...
<<<<<<< HEAD:code/power demand.R

=======
>>>>>>> e27d1acc17ff1403bee2eeeed6788ecd414b59f5:power demand.R
